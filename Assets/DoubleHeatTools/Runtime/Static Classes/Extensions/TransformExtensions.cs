...
namespace DoubleHeat.Utilities {
    ...
    public static class TransformExtensions {
        ...
        public static Vector3 TransformPointOfParent (this Transform transform, Vector3 p) {
            if (transform.parent) {
                return transform.parent.TransformPoint(p);
            }
            return p;
        }

        public static Vector3 TransformVectorOfParent (this Transform transform, Vector3 v) {
            if (transform.parent) {
                return transform.parent.TransformVector(v);
            }
            return v;
        }

        public static Vector3 TransformDirectionOfParent (this Transform transform, Vector3 dir) {
            if (transform.parent) {
                return transform.parent.TransformDirection(dir);
            }
            return dir;
        }

        public static Vector3 InverseTransformPointOfParent (this Transform transform, Vector3 p) {
            if (transform.parent) {
                return transform.parent.InverseTransformPoint(p);
            }
            return p;
        }

        public static Vector3 InverseTransformVectorOfParent (this Transform transform, Vector3 v) {
            if (transform.parent) {
                return transform.parent.InverseTransformVector(v);
            }
            return v;
        }

        public static Vector3 InverseTransformDirectionOfParent (this Transform transform, Vector3 dir) {
            if (transform.parent) {
                return transform.parent.InverseTransformDirection(dir);
            }
            return dir;
        }

        public static Quaternion ConvertLocalRotationToWorldRotation (this Transform transform, Quaternion localRotation) {
            if (transform.parent) {
                return localRotation * transform.parent.rotation;
            }
            return localRotation;
        }

        public static Quaternion ConvertWorldRotationToLocalRotation (this Transform transform, Quaternion rotation) {
            if (transform.parent) {
                return rotation * Quaternion.Inverse(transform.parent.rotation);
            }
            return rotation;
        }
